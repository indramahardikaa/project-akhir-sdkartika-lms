...
import { getUsers, createUser, deleteUser } from '@/lib/data';
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-gray-800">Registrasi User</h2>
        ...
        {showForm && (
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            <h3 className="font-bold text-gray-800 mb-4">Tambah User Baru</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input placeholder="Password" value={form.password} onChange={e => setForm({...form, password: e.target.value})} className="border rounded-lg px-3 py-2 text-sm" />
              <select value={form.role} onChange={e => setForm({...form, role: e.target.value as 'guru' | 'siswa'})} className="border rounded-lg px-3 py-2 text-sm">
              ...
              {users.map(u => (
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    {u.role !== 'admin' && <button onClick={() => handleDelete(u.id)} className="text-accent-600 hover:text-accent-800 text-xs font-medium">Hapus</button>}
                  </td>
                </tr>
              ))}
            </tbody>
