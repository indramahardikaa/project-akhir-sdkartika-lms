...
import { getCoursesByGuru, getMaterials, createMaterial, deleteMaterial } from '@/lib/data';
...
export default function GuruMaterialsPage() {
  ...
  const loadData = () => {
    ...
    setMaterials(getMaterials().filter(m => courseIds.includes(m.courseId)).map(m => ({ ...m, courseName: c.find(x => x.id === m.courseId)?.title || '-' })));
  };
  ...
  const handleDelete = (id: string) => {
    if (confirm('Hapus materi?')) { deleteMaterial(id); loadData(); }
  };
            ...
            <tbody className="divide-y">
              {materials.map(m => (
                <tr key={m.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3"><button onClick={() => handleDelete(m.id)} className="text-accent-600 text-xs font-medium">Hapus</button></td>
                </tr>
              ))}
            </tbody>
          </table>
